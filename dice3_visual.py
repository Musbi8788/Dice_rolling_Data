...
# Analyze the results
frequencies = []
max_result = die_1.num_sides + die_2.num_sides + die_3.roll() # join the two dies result 12
for value in range(3, max_result+1): # loop through the 12 side
    frequency = results.count(value) # count how many time a number repeated
    frequencies.append(frequency) # store the frequency 

# Visualize the results
x_values = list(range(3, max_result+1)) # convert the range in a list and display the result on the vertical of the chart.

data = [Bar(x=x_values, y=frequencies)] # set the data in a bar chart 
title = 'Result of rolling a three D6 1000 times.'
...
offline.plot({'data': data, 'layout':my_layout}, filename='d6_d6_dt.html') # generate a chart and save it in a html format
